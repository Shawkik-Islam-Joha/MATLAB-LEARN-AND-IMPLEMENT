...
%% FWHM values from Table 2
% These are the FWHM values reported in the paper.
FWHM = [2.01 2.67 2.49 3.03 3.84 4.26];
...
for k = 1:6
    ...
    R_healthy = reflectance(theta1, lambda, n1, n2, d2, ...
                            n3, d3, n4, d4, n5_healthy(k));

    R_cancer  = reflectance(theta1, lambda, n1, n2, d2, ...
                            n3, d3, n4, d4, n5_cancer(k));

    %% Find resonance angles
    [~, idxH] = min(R_healthy);
    [~, idxC] = min(R_cancer);

    theta_res_healthy = theta_deg(idxH);
    theta_res_cancer  = theta_deg(idxC);

    %% Calculate changes
    delta_theta = abs(theta_res_cancer - theta_res_healthy);
    delta_n = n5_cancer(k) - n5_healthy(k);

    %% Performance parameters
    S  = delta_theta / delta_n;
    QF = S / FWHM(k);
    DA = delta_theta / FWHM(k);

    %% Plot
    subplot(3,2,k);

    plot(theta_deg, R_healthy, 'b-', 'LineWidth', 1.5);
    hold on;
    ...
    legend('Healthy','Cancerous','Location','southeast');
